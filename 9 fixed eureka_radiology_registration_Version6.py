...
class PatientRegistrationApp:
    ...
    def _on_new_modality_file(self, filepath, modality_code):
        ...
        logging.info(f"Staging new modality file for attachment: {fname} (modality={modality_code or 'General'})")
        ...
        if filepath not in self.manual_attachments:
            ...
            self.update_status(f"Auto-staged for attachment: {fname}", duration=4000)
            logging.info(f"File {fname} added to manual_attachments. Total: {len(self.manual_attachments)}")
        else:
            logging.info(f"File {fname} was already in manual_attachments. Skipping auto-stage.")
