                ...
                if os.path.exists(prospective_snapshot_path):
                    ...
                    if snapshot_size > 0:
                        if snapshot_size == original_size:
                            ...
                            logging.warning(f"_process_file: Snapshotted {original_watched_path} to {prospective_snapshot_path}, but size MISMATCH! Original: {original_size}, Snapshot: {snapshot_size}")
...
class PatientRegistrationApp:
    ...
    def _on_new_modality_file(self, local_snapshot_filepath, modality_code): # Parameter name changed for clarity
        ...
        if local_snapshot_filepath not in self.manual_attachments:
            ...
            self.update_status(f"Auto-staged for attachment: {original_fname_for_dest}", duration=4000)
    ...
    def process_attachments(self, patient_id_for_match, modality_code, patient_specific_report_folder):
        ...
        if watch_folder and os.path.isdir(watch_folder):
            ...
            try:
                ...
                if copied_misc > 0:
                    self.update_status(f"Copied {copied_misc} DICOM file(s) from watch folder.")
            except Exception as e:
    ...
    def open_settings_window(self):
        ...
        # Bind folder selection to load auto-send configuration
        ...
        # Auto-send configuration section for external reports
        ...
        # Placeholders info
        ttk.Label(autosend_frame, text=self.get_ui_label("ext_report_autosend_placeholders_label", "Placeholders: {Filename}, {FolderPath}, {DateTime}, {Patient Name}, {Modality}, {Study Description}"),
    ...
    def _on_ext_report_folder_select(self, event=None):
        """Load auto-send configuration when a folder is selected"""
    ...
    def _process_auto_send_for_external_report_file(self, file_path):
        """Process auto-send email for external report files with patient data extraction"""
        ...
        # Extract patient data from PDF filename
        ...
        # Replace placeholders with extracted patient data and file info
...
class ServedWorklistDialog(tk.Toplevel):
    def __init__(self, parent, app_config, palette, get_ui_label_func, main_app_ref):
        ...
        # Make modality column read-only by setting it to a different color
